...
## TASK (S3 storyboard + script review, table-read round 1)

You are reviewing the S3 artifacts of the Night School session "dotprod" before any production. Below: spine.md (the session constitution), storyboard.yaml (the per-beat contract), script.md (the video narration with [visual:] cues), and the R1-R30 digest you verdict against. The mechanical linters (L1-L12, V1) have already passed; do not re-flag their letter, only gamed intent. Emit ONLY the JSON array of your output contract. No code fences, no commentary: raw JSON. An empty array is a pass. Do not use any tools; everything you may see is in this message.
...
beats:
  - id: beat-01
    ...
    narration: "Your music app just played you a song you love, by a band you have
      never heard of. Two descriptions got compared in a data center and the
      verdict was: same taste. But sit with what that comparison has to do. A
      song is not one number. It is loud, fast, sad, bright, many qualities at
  ...
  - id: beat-02
    ...
    narration: "You just tried to score that match by hand, and it fought back:
      too many qualities, no clean way to fold them into one number. Here is the
      move that lands it. A taste is an arrow: each direction is one quality,
      and the length is how hard the song leans that way. The recipe is almost
      embarrassingly small. Multiply the two arrows entry by entry, then add.
      If your taste is two and one, and a song is one and three, that is two
      times one plus one times three: five. One number, the whole tangle
      collapsed. Now meet the instrument. One probe arrow, the thing doing the
      asking. Four candidates, holding still. Under each candidate, a bar
      showing its current score. Before anything moves, call your shot, and
