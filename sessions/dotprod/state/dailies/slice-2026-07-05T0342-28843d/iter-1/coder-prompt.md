...
Beat:
- id: beat-04
- title: What the meter gets wrong
- narration (spoken, NEVER on screen): Time to close the loop. Rebuild the meter from memory: two arrows go in, so what two operations turn them into the score?
...
- visual intent: [visual: dynamic-process | connect-to-reality] The retrieval prompt holds as a card while the meter idles. The song columns swap for two labeled "you" and "the role": Python hours, SQL reps, nights free; matched entries multiply and pool into the same lilac readout on the same dial. A guess card holds with one candidate outlined and the score readout hidden. Then that periwinkle candidate stretches longer; its lilac score climbs while its direction holds still; a small rose flag marks the unearned win. The walk-home question lands as the closing card, and a quiet chip points to the extensions menu on the page.
