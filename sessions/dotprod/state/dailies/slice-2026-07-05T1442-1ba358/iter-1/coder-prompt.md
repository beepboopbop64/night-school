...
Beat:
- id: beat-04
- title: Same meter, different aisle
...
- visual intent: [visual: dynamic-process | connect-to-reality] The card holds the actual question in compressed form, "Rebuild the meter from memory. Which two operations?", while the meter idles. The song columns swap for two labeled "you" and "the role": Python hours, SQL reps, nights free, six rows at full readable size with the whole frame to breathe; matched entries multiply and pool into the same lilac readout on the same dial, and "two arrows in, one number out" lands as the readout label. "Different aisle, same meter" lands as the closing line.
