...
- visual intent: [visual: dynamic-process | connect-to-reality] The retrieval prompt holds as a card while the meter idles. The song columns swap for two labeled "you" and "the role": Python hours, SQL reps, nights free, six rows at full readable size with the whole frame to breathe; matched entries multiply and pool into the same lilac readout on the same dial.
