...
11. **Revisions preserve interaction feel.** When revising an existing
    component, never change interaction resolution, control order, or event
    wiring unless a failing check demands it. The spec's `step` governs
    ARROW KEYS; finer pointer (drag) resolution is allowed and desirable.
    Do not coarsen a smooth drag to the keyboard step.
...
<script lang="ts">
	...
	// the arithmetic for whichever candidate you select, live. The meter's
	// score prints to one decimal (so is the strip's final score, same
	// quantity, same precision); the coordinate and product terms print to
	// two, since those are a different quantity.
	...
	// Leadership is judged at the SAME precision the meter prints (one
	// decimal): two scores that display the same digit must never crown
	// one of them the leader, per the displayed-precision rule.
	const displayScore = (n: number) => Math.round((Object.is(n, -0) ? 0 : n) * 10) / 10;
	const maxDisplay = $derived(Math.max(...rows.map((r) => displayScore(r.score))));
	const leaders = $derived(rows.filter((r) => displayScore(r.score) === maxDisplay).map((r) => r.id));
	...
	const cheatVictim = $derived.by(() => {
		...
		const beaten = rows.filter(
			(r) =>
				...
				displayScore(c1.score) > displayScore(r.score)
		);
	...
	<div class="stage">
		<div class="arena-wrap">
			...
			>
				...
				{/each}
				<!-- candidate tip tags: c1..c4, always on and radially clear of the
				     sweeping probe, so the id never gets buried mid-turn. Their
				     story names live in the stable meter cards below. -->
				{#each rows as r (r.id)}
					...
					<text
						...
						class:sel={selected === r.id}
						aria-hidden="true">{r.id}</text>
				{/each}

				<!-- probe on top; its own tip tag is nudged off-angle so it never
				     lands on top of a candidate's tag even when the probe points
				     straight at that candidate. -->
				{#if true}
					{@const a = arrow(probe.x, probe.y)}
					{@const plp = labelPos(probeAngle + 13, 1, 34)}
...
<style>
	...
	.song-lab { font-size: 11.5px; opacity: 0.7; }
	.song-lab.sel { opacity: 1; font-weight: 600; }
...
--- FEEDBACK (verify-first: the existing component failed these checks as-is, before any coder changes this run) ---
  ...
  "weba11y": [
    {
      ...
      "target": "slider",
      "message": "a pointer drag across the slider did not change its value (before=40, after=40); the control must be pointer-operable"
    }
  ]
}
