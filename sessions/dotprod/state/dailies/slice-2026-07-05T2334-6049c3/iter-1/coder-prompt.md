...
- visual intent: [visual: connect-to-reality] Two tall stacks of qualities slide in side by side, one for the listener's taste and one for the song, each row a named feature in periwinkle: loud, fast, sad, bright, and more trailing off the frame. A short cream ruler lays against one row at a time and never spans the stack, and a run of exact-match ticks flickers out as rows refuse to agree. A single lilac match readout waits with a question mark where its number should be. The question card holds over the unfilled readout, half the frame deliberately empty so the puzzle, not an answer, owns the screen.
