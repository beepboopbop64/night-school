...
## TASK (S3 script cold-read, table-read round 4; you have not seen any prior round)

Below is the complete script of a Night School session video. Narration paragraphs are what you would hear. The bracketed [visual:] paragraphs are the video's stage directions describing what would be on screen while those words play; the leading words inside each bracket (connect, covary, and so on) are direction shorthand for the kind of motion, which is normal script formatting, not production material. Read it ONCE, straight through, as the student you are. Then follow your protocol and emit ONLY the JSON object of your output contract. Probe administration note: the session's closing retrieval prompt is the 'Rebuild it from memory' question near the end; the final walk-home question is the next session's teaser, not the retrieval prompt. No code fences, no commentary, no markdown: raw JSON.
