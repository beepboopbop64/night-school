...
## TASK (S3 voice pass, table-read round 2, Sonnet tier)

Copy under review: (1) the video narration script; [visual:] paragraphs are production directions, NOT learner-facing; judge only the spoken narration paragraphs; (2) the learner-facing storyboard copy (titles, on-screen text, prediction prompts, canonical strings). The mechanical dash lint (V1) has already passed. Two Showrunner rulings already on record, do not re-flag: (a) the hook's 'my favorite desperate hack' is licensed first-person taste, allowed unless it reads as costume in context; (b) beat-03's landing line 'Alignment is the score. Your hands already know it.' is the locked landing line, kept with rationale (the page twin makes it literal). Emit ONLY the JSON array of line-level verdicts per your output contract (an empty array is a pass). No code fences, no commentary: raw JSON.
