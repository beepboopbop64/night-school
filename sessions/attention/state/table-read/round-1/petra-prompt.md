...
## TASK (S3 storyboard + script table-read, round 1)
...
beats:
  - id: beat-01
    ...
    narration: >-
      ...
      the source and the line you're writing. So keep every word, and for
      each word the model writes, let it look back. So why did nobody just
      build it?
    visual: >-
  ...
  - id: beat-02
    ...
    narration: >-
      ...
      zero everywhere you can stand, and training means following slopes. So
  ...
  - id: beat-03
    ...
    narration: >-
      Strip the problem to its smallest piece. Forget translation. You're mid
      sentence and you need cat flavored information. One probe vector, four
      stored candidates. Before it moves, call the order the bars will peak.
      A query is a question you ask every key at once. Sweep it across the
      row and each dot product answers out loud: point the same way and the
      score climbs, point away and it dies. Now the candidates take names:
      cat, kitten, car, sofa. Park the probe near cat and read the scores.
      Four, two, zero, minus four. You might think a high score means two
      tokens are the same kind of thing. But the keys never moved. Every
      score rose and died with the probe's direction alone. Alignment in a
      learned space, not sameness. Alignment is the score. Your hands already
      know it.
    visual: >-
  ...
  - id: beat-04a
    ...
    narration: >-
      ...
      obvious machine scores every key, takes the winner, returns its card.
      Cat wins. Correct. Rearrange it: keys left, cards right. You know this
      object. It's a Python dict. Ask for cat, instant answer. Ask for
      feline, KeyError, one synonym from a key it's literally holding. Our
      version scores the near miss, kitten earned a two, and the pick is
      like, kitten, noted... anyway, one hundred percent cat. Now try to
      learn. Wiggle the query. Winner unchanged, output unchanged, gradient
      zero. Push further and the output teleports. The staircase is back,
      with numbers on it. That's not a failure of effort. It's the shape of
      the function. Keep the ranking. Lose the cliff. What's the smallest
      edit you can make?
    visual: >-
  ...
  - id: beat-04b
    ...
    narration: >-
      ...
      so they total one. Blend the cards in exactly those proportions. The
      answer is mostly cat, a real slice of kitten, almost nothing else. Now
      wiggle the query. Weights slide, output slides, the meter is alive. And
      wait. You know these bars. Point seven seven on cat, so cat is
      seventy seven percent of the why. Right? Hold that thought. The lookup
      didn't fail to answer. It failed to learn.
    visual: >-
  ...
  - id: beat-05
    ...
    narration: >-
      ...
      before. Where? The dict from earlier slides in beside it. Walk the
      rows. Exact match becomes a continuous score. Return one value becomes
      return a blend. KeyError becomes impossible: the weights just spread.
      Row by row, the dict you use every day becomes the machine you just
      built. Attention is a soft lookup table. And those lilac bars? A recipe
      card: what got mixed, never why the recipe was chosen. The choosing
      lives upstream, in the scoring that learned to build those numbers.
      This blend is what the intern implemented, and it worked on the first
      try. The curve from the opening completes: over the collapsing line,
      the rescue draws in flat.
    visual: >-
  ...
  - id: beat-06a
    ...
    narration: >-
      ...
      summed: that's the blend. The weights: softmax of the scores. That's
      the exponentiate and normalize you already did. It had a name. Smooth
      argmax, picks the winner? No. Cat got point seven seven, not one, and
      kitten is measurably in the answer. Nothing was discarded. Now the bill
      for that square root. Wide vectors make big dot products, big scores
      freeze softmax near one hot, and a frozen softmax has a slope near
      zero. Dividing by the square root of the width keeps the same
      preference and a living gradient. Which keeps learning healthy,
  ...
  - id: beat-06b
    ...
    narration: >-
      The sentence is a claim, so try to break it. Multiply every score by
      ...
      gradient dies: weight times one minus weight, zero. Quick honesty: the
      sharpening dial is softmax's, not the transformer's; its fixed cousin
      is that square root. And 2014 scored with a tiny network, not dot
      products; those came later. So the hard lookup was never wrong. It was
      unlearnable. Gradient flows only while the lookup stays soft.
    visual: >-
  ...
  - id: beat-07
    ...
    narration: >-
      One more test, on the thing you run at work: RAG. Embed the question:
      a query vector. Compare against a vector index: keys. Fetch passages:
      values. Map it yourself. Same skeleton, and the differences are the
      lesson. RAG's lookup is hard and external, a top k you can't backprop
      through. Attention's is soft, internal, learned end to end. Opposite
      ends of one dial. One aside: chat models also mask future keys before
      the softmax; the machine is otherwise unchanged. Now, that splinter
      from earlier. Forty percent of the attention went to the word
      terrible, so terrible is forty percent of why the review read
      negative. True or false? In 2019, on BiLSTM classifiers, not
      transformers, very different attention maps produced the same
      predictions and correlated weakly with other importance measures. The
      follow-up debate is honestly unresolved. So a heatmap sold as why is
      really a recipe: what got mixed, never why it was chosen. Same
  ...
  - id: beat-08
    ...
    narration: >-
      Before the door: rebuild it from memory. Three objects, two
      operations. What replaced the exact match, and what replaced returning
      one value? While you think, watch the stored pairs shuffle. The
      weights follow them. The output doesn't move a digit. A dictionary has
      no idea what order you wrote it in. Neither does attention. The cat
      sat on the mat. The mat sat on the cat. Same tokens, same keys, same
      values, same blend. Your language can tell them apart. This machine
      can't. So, for the walk home: what's the cheapest thing you could add
      to a token so that where it sits becomes something a lookup can find?
      Two years after that September, Google Translate went neural in
      production, half a billion users, a hundred billion words a day. Three
      years after, Vaswani and colleagues threw away the recurrent network
      and kept only the fix. The fix outlived the architecture it rescued.
      And the amber dot from the opening, Bahdanau's own gaze: session two
      gives it the one thing it still lacks. Where the words sit.
    visual: >-
      ...
      notes the extensions menu ("Still up?") on the page. Final image: the
      amber gaze dot from beat-01 hops once more between source and target,
      then parks on the unanswered question.
    tracks: [video, page]
    newIdea: >-
