...
## TASK (S3 voice pass, table-read round 1, Sonnet tier)

Copy under review: (1) the video narration script; [visual:] paragraphs are production directions, NOT learner-facing; judge only the spoken narration paragraphs; (2) the learner-facing storyboard copy (titles, on-screen text, prediction prompts, canonical strings). The mechanical dash lint (V1) has already passed. Watch-item from the Showrunner: the hook's phrase 'my favorite desperate hack' is licensed first-person taste, ruled allowed (not a fabricated anecdote); flag it ONLY if it reads as costume in context. Emit ONLY the JSON array of line-level verdicts per your output contract (an empty array is a pass). No code fences, no commentary: raw JSON.
