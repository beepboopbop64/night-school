...
- narration (spoken, NEVER on screen): A query is a question you ask every key at once. Sweep it across the row and each dot product answers out loud: point the same way and the score climbs, point away and it dies.
- visual intent: [visual: covary | dynamic-concept] One amber query vector (labeled q, on the object) rotates through a range of directions while a row of four periwinkle key vectors (labeled k1..k4 at their tips) holds still. Under each key sits a lilac score bar with a numeric readout: its height rises and falls WITH the query's alignment to that key, live, as the query sweeps. At any moment the best-aligned key's bar visibly leads and the query's motion is the single focus; keys and bars recede when not changing. No legends, no swatch keys: every label sits on its object. The covariation (query direction <-> score height) must be readable from the frames alone.
- colors (semantic cast):
  - query: data.heat (colors.py constant DATA_HEAT)
  - keys: data.observed (colors.py constant DATA_OBSERVED)
...
Files and names:
- Write the scene module at: C:\Users\jaker\Desktop\projects\harnesses\lessons\sessions\slice-attention\video\scenes\scene_qk_similarity.py
- The module must define exactly one Scene subclass named: QkSimilarity
...
REPLAN REQUIRED: the same defect has recurred; incremental patching has
failed. Follow the forced re-plan protocol: FIRST write a fresh layout
plan to C:\Users\jaker\Desktop\projects\harnesses\lessons\sessions\slice-attention\video\scenes\scene_qk_similarity.plan.md, THEN rewrite the scene module from scratch to
implement it.
...
{
  "iteration": 2,
  "renderError": "manim render -ql --disable_caching --progress_bar none --output_file scene_qk_similarity --media_dir C:\\Users\\jaker\\Desktop\\projects\\harnesses\\lessons\\sessions\\slice-attention\\media C:\\Users\\jaker\\Desktop\\projects\\harnesses\\lessons\\sessions\\slice-attention\\video\\scenes\\scene_qk_similarity.py QkSimilarity exited 1:\n|    737             # Return a bound method                                  |\n|    738             return types.MethodType(getter, self)                    |\n|                                                                             |\n| C:\\Users\\jaker\\AppData\\Local\\Programs\\Python\\Python313\\Lib\\site-packages\\ma |\n| nim\\mobject\\mobject.py:760 in __getattr__                                   |\n|                                                                             |\n|    757             return types.MethodType(setter, self)                    |\n|    758                                                                      |\n|    759         # Unhandled attribute, therefore error                       |\n| >  760         raise AttributeError(f\"{type(self).__name__} object has no a |\n|    761                                                                      |\n|    762     @property                                                        |\n|    763     def width(self) -> float:                                        |\n+-----------------------------------------------------------------------------+\nAttributeError: ValueTracker object has no attribute 'fill_opacity'"
}
